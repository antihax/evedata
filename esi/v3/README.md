...
## New Client
```
  client, err := esiv3.NewClient(*http.Client, userAgent string)
```
...
```
  client, err := esiv3.NewClient(nil, "esiv3 client http://mysite.com")
...
```
  ...
  client, err := esiv3.NewClient(nil, "esiv3 client http://mysite.com")
  ctx := context.WithValue(context.TODO(), esiv3.ContextOAuth2, auth)
