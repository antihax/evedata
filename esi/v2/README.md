...
## New Client
```
  client, err := esiv2.NewClient(*http.Client, userAgent string)
```
...
```
  client, err := esiv2.NewClient(nil, "esiv2 client http://mysite.com")
...
```
  ...
  client, err := esiv2.NewClient(nil, "esiv2 client http://mysite.com")
  ctx := context.WithValue(context.TODO(), esiv2.ContextOAuth2, auth)
