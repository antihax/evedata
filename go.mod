module github.com/antihax/evedata

go 1.16

require (
	github.com/FZambia/go-sentinel v0.0.0-20171204085413-76bd05e8e22f
	github.com/Masterminds/squirrel v1.5.1
	github.com/RyanCarrier/dijkstra v1.1.0
	github.com/antihax/goesi v0.0.0-20211028231550-640b98e35e09
	github.com/antihax/httpcache v0.0.0-20180513210545-d834ed68b3ae
	github.com/antihax/redistore v0.0.0-20170226192907-7d9063dbaeb3
	github.com/bradfitz/slice v0.0.0-20180809154707-2b758aa73013
	github.com/bwmarrin/discordgo v0.23.2
	github.com/cespare/xxhash/v2 v2.1.2 // indirect
	github.com/coreos/go-oidc/v3 v3.1.0
	github.com/garyburd/redigo v1.6.2
	github.com/ghodss/yaml v1.0.0
	github.com/go-sql-driver/mysql v1.6.0
	github.com/golang/snappy v0.0.4 // indirect
	github.com/gorilla/context v1.1.1
	github.com/gorilla/mux v1.8.0
	github.com/gorilla/sessions v1.2.1
	github.com/gorilla/websocket v1.4.2 // indirect
	github.com/guregu/null v4.0.0+incompatible
	github.com/jmoiron/sqlx v1.3.4
	github.com/nsqio/go-nsq v1.1.0
	github.com/prometheus/client_golang v1.11.0
	github.com/prometheus/common v0.32.1 // indirect
	github.com/prometheus/procfs v0.7.3 // indirect
	github.com/stretchr/testify v1.7.0
	go4.org v0.0.0-20201209231011-d4a079459e60 // indirect
	golang.org/x/crypto v0.0.0-20210921155107-089bfa567519
	golang.org/x/oauth2 v0.0.0-20211028175245-ba495a64dcb5
	golang.org/x/text v0.3.8
	google.golang.org/genproto v0.0.0-20211028162531-8db9c33dc351 // indirect
	google.golang.org/grpc v1.41.0
	gopkg.in/mgo.v2 v2.0.0-20190816093944-a6b53ec6cb22
	gopkg.in/square/go-jose.v2 v2.6.0 // indirect
	gopkg.in/vmihailenco/msgpack.v2 v2.9.2
	gopkg.in/yaml.v2 v2.4.0
	gopkg.in/yaml.v3 v3.0.0-20210107192922-496545a6307b // indirect
)
