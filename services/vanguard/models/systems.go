...
func GetSystemCelestials(system int32) ([]SystemCelestials, error) {
	...
	coalesce(D.radius,0) AS radius,
	coalesce(D.celestialIndex,0) AS celestialIndex, 
	coalesce(D.orbitIndex,0) AS orbitIndex
		FROM mapDenormalize D
		LEFT OUTER JOIN invGroups G ON G.groupID = D.groupID
