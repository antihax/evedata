...
func (s *MarketCollector) saveContractItems(id int32, c []esi.GetContractsPublicItemsContractId200Ok) error {
	if len(c) > 0 {
...
func (s *MarketCollector) saveContractAdditions(c []marketwatch.FullContract) {
...
func (s *MarketCollector) saveContractChanges(c []marketwatch.ContractChange) {
	for _, g := range c {
		if g.Type_ == "auction" {
